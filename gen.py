...
def run_gen_on_posix(args):
    ...
    run('cmake'
        ' -DCMAKE_BUILD_TYPE={}'
        ' -DEZIO_BUILD_UNITTESTS={}'
        ' -DEZIO_BUILD_EXAMPLES={}'
        ' -G "{}"'
        ' {}'
        .format(build_type,
                boolean_to_option(not args.build_no_test),
                boolean_to_option(not args.build_no_examples),
                generator,
                CMAKE_ROOT))
...
def parse_args():
    ...
    parser.add_argument('--no-examples', dest='build_no_examples', action='store_true')
